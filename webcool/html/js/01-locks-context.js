(function () {
  ...
  window.WebCoolModuleRegistry.define("01-locks-context.js", [
    "",
    ...
    "        }",
    "        if (action === 'paste') {",
    "          if (!remoteDiskClipboardPath) {",
    "            showStatus(t('没有可粘贴的远程文件或目录'), 'err');",
    "            return;",
    "          }",
    "          if (remoteDiskClipboardDirectory && isSameOrChildFolderPath(remoteDiskClipboardPath, path)) {",
    ...
    "            return;",
    "          }",
    "          const buildCopyUrl = function (overwrite) {",
    "            let url = remoteDiskClipboardDirectory",
    "              ? (api.folderCopy + '?async=1&path=' + encodeURIComponent(remoteDiskClipboardPath) + '&folder=' + encodeURIComponent(path))",
    "              : (api.fileCopy + '?async=1&file=' + encodeURIComponent(remoteDiskClipboardPath) + '&folder=' + encodeURIComponent(path));",
    "            if (overwrite) {",
    "              url += '&overwrite=1';",
    "            }",
    "            if (remoteDiskClipboardDirectory) {",
    "              url = withFolderPassword(url, remoteDiskClipboardPath);",
    "            } else {",
    "              url = withFolderPassword(url, parentFolderPathFromFilePath(remoteDiskClipboardPath));",
    "              url = appendFilePassword(url, remoteDiskClipboardPath, false);",
    "            }",
    "            return withFolderPassword(url, path, 'target_folder_password');",
    "          };",
    "          let result = null;",
    "          try {",
    "            result = await fetchJson(buildCopyUrl(false), { method: 'POST' });",
    "          } catch (err) {",
    ...
    "                return;",
    "              }",
    "              result = await fetchJson(buildCopyUrl(true), { method: 'POST' });",
    "            } else {",
    "              throw err;",
    "            }",
    "          }",
    "          const copiedPath = String((result && result.path) || '');",
    "          const copiedDirectory = remoteDiskClipboardDirectory;",
    "          closeFolderContextMenu();",
    "          closeFileContextMenu();",
    "          setRemoteCopyProgress(0, t('准备粘贴...'), { name: copiedPath || remoteDiskClipboardPath, state: 'running', progress: 0, size: 0, copied: 0 });",
    "          const copyResult = await pollRemoteCopyTask(String((result && result.task_id) || ''), path, copiedPath, copiedDirectory);",
    "          if (String((copyResult && copyResult.state) || '') === 'done' && (copiedDirectory || copiedPath)) {",
    "            showStatus(t('已粘贴到：') + (copiedPath || path), 'ok');",
    "          }",
    "          return;",
    "        }",
    "        if (action === 'lock') {",
    ...
    "        }",
    "        if (action === 'copy') {",
    "          localDiskClipboardPath = dirPath;",
    "          localDiskClipboardDirectory = true;",
    "          showStatus(t('已拷贝本地目录路径：') + dirPath, 'ok');",
    "          return;",
    "        }",
    "        if (action === 'upload') {",
    ...
    "            return;",
    "          }",
    "          if (localDiskClipboardDirectory && localDiskPathContains(localDiskClipboardPath, dirPath)) {",
    "            showStatus(t('不能将目录粘贴到自身或其子目录中'), 'err');",
    "            return;",
    "          }",
    "          if (locked && !getLocalDirPassword(dirPath) && !(await ensureLocalDirUnlocked(dirPath))) {",
    "            return;",
    "          }",
    "          const sourceLockPath = localDiskClipboardDirectory && getLocalDirPassword(localDiskClipboardPath)",
    "            ? localDiskClipboardPath",
    "            : localDiskParentPath(localDiskClipboardPath);",
    "          const buildCopyUrl = function (overwrite) {",
    "            let url = api.localDiskCopy",
    "              + '?path=' + encodeURIComponent(localDiskClipboardPath)",
    "              + '&target=' + encodeURIComponent(dirPath);",
    "            if (localDiskClipboardDirectory) {",
    "              url += '&async=1';",
    "            }",
    "            if (overwrite) {",
    "              url += '&overwrite=1';",
    ...
    "                appendFilePassword(url, localDiskClipboardPath, true),",
    "                sourceLockPath",
    "              ),",
    "              dirPath,",
    ...
    "          }",
    "          const copiedPath = String((result && result.path) || '');",
    "          const copiedDirectory = localDiskClipboardDirectory;",
    "          const copiedItemName = copiedPath || localDiskClipboardPath;",
    ...
    "            if (String((copyResult && copyResult.state) || '') === 'done' && (copiedDirectory || copiedPath)) {",
    "              showStatus(t('已粘贴到：') + (copiedPath || dirPath), 'ok');",
    "            }",
    "            return;",
    "          }",
    "          localDiskClipboardPath = '';",
