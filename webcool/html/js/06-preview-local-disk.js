(function () {
  ...
  window.WebCoolModuleRegistry.define("06-preview-local-disk.js", [
    ...
    "            '</video>' +",
    "            ((!opts.local && isRealMediaVideoName(rawName))",
    "              ? '<div class=\"preview-video-tools\"><button class=\"preview-convert-btn\" type=\"button\" data-preview-convert-video=\"' + encodeURIComponent(rawName) + '\">' + t('转换为MP4') + '</button><span>' + t('RM/RMVB格式浏览器兼容性较差，可转换为MP4后播放。') + '</span></div>'",
    "              : '');",
    "        } else if (kind === 'audio') {",
    ...
    "              showManualTranscodePrompt([{ name: rawConvertFile, reason: t('RM/RMVB格式浏览器兼容性较差，可转换为MP4后播放。') }]);",
