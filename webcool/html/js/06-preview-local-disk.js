(function () {
  ...
  window.WebCoolModuleRegistry.define("06-preview-local-disk.js", [
    ...
    "              ? '<div class=\"preview-video-tools\"><button class=\"preview-convert-btn\" type=\"button\" data-preview-convert-video=\"' + encodeURIComponent(rawName) + '\" data-preview-convert-local=\"' + (opts.local ? '1' : '0') + '\">' + t('转换为MP4') + '</button><span>' + (opts.local ? t('本地磁盘RM/RMVB/AVI格式可转换为MP4，输出文件会保存在源文件相同目录。') : t('RM/RMVB格式浏览器兼容性较差，可转换为MP4后播放。')) + '</span></div>'",
    "              : '');",
    "        } else if (kind === 'audio') {",
