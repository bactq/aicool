(function () {
  ...
  window.WebCoolModuleRegistry.define("03-transcode-upload.js", [
    ...
    "          statusBox.innerHTML =",
    "            '<div>' + t('检测到以下视频为了兼容浏览器建议转换为MP4，请确认是否转换：') + '</div>' +",
    ...
    "          title: t('转换为MP4'),",
    "          description: t('上传完成，检测到 ') + list.length + t(' 个视频建议转换为MP4以兼容浏览器播放。是否现在转换？'),",
    "          confirmText: t('转换为MP4'),",
    "          cancelText: t('保持原文件'),",
    "          danger: false",
    "        });",
    "        if (!confirmed) {",
    "          showStatus(t('已保留原视频文件，稍后播放时仍可转换为MP4。'), 'warn');",
    "          return;",
    "        }",
    ...
    "          if (name) {",
    "            startManualTranscode(encodeURIComponent(name));",
    "          }",
    "        }",
    "      }",
    "",
